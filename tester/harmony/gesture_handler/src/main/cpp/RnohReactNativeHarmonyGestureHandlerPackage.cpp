...
using FactoryDelegate = ComponentInstanceFactoryDelegate::Shared;
...
FactoryDelegate RnohReactNativeHarmonyGestureHandlerPackage::createComponentInstanceFactoryDelegate()
{
...
public:
    void handleArkTSMessage(const Context &ctx) override
    {
        ...
        if (ctx.messageName == "RNGH::SET_NATIVE_RESPONDERS_BLOCK") {
            handleSetNativeRespondersBlock(ctx);
        } else if (ctx.messageName == "RNGH::ROOT_VIEW_IS_HANDLING_TOUCHES") {
            handleRootViewIsHandlingTouches(ctx);
        }
    }

private:
    void handleSetNativeRespondersBlock(const Context &ctx)
    {
        ...
        auto ancestors = getComponentAncestors(rnInstanceCAPI, targetComponentInstanceTag);
        if (ancestors.empty()) {
            return;
        }
        setNativeResponderBlockStatus(ancestors, shouldBlock);
    }
    
    void handleRootViewIsHandlingTouches(const Context &ctx)
    {
        ...
        updateRootViewHandlingStatus(rnInstanceCAPI, descendantViewTag, isHandlingTouches);
    }
    
    std::vector<ComponentInstance::Shared> getComponentAncestors(
        std::shared_ptr<RNInstanceCAPI> rnInstanceCAPI,
        double targetComponentInstanceTag)
    {
    ...
    void setNativeResponderBlockStatus(
        const std::vector<ComponentInstance::Shared> &ancestors,
        bool shouldBlock)
    {
        bool isChangingResponderStatusAllowed = false;
    ...
    void updateRootViewHandlingStatus(
        std::shared_ptr<RNInstanceCAPI> rnInstanceCAPI,
        double descendantViewTag,
